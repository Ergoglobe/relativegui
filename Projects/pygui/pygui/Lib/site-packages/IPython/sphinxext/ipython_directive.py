...
def block_parser(part, rgxin, rgxout, fmtin, fmtout):
    ...
    while 1:
        ...
        if any(
            line_stripped.startswith(f"@{pseudo_decorator}")
...
class EmbeddedSphinxShell(object):
    ...
    def process_image(self, decorator):
        ...
        outfile = f'/{path.relative_to(source_dir).as_posix()}'
    ...
    def process_input(self, data, input_prompt, lineno):
        ...
        if not is_okexcept and (
            ...
            if self.warning_is_error:
                raise RuntimeError(f"Unexpected exception in `{filename}` line {lineno}")
        ...
        if not is_okwarning:
            for w in ws:
                ...
                logger.warning(s)
                if self.warning_is_error:
                    raise RuntimeError(f"Unexpected warning in `{filename}` line {lineno}")
