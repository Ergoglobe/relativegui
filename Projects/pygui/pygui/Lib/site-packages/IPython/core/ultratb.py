...
def _format_traceback_lines(lines, Colors, has_colors: bool, lvals):
    ...
    for stack_line in lines:
        ...
        if stack_line.is_current:
            ...
            num = f'{debugger.make_arrow(pad)}{str(lineno)}'
            start_color = Colors.linenoEm
        else:
            num = '%*s' % (numbers_width, lineno)
            start_color = Colors.lineno

        line = f'{start_color}{num}{Colors.Normal} {line}'
...
class TBTools(colorable.Colorable):
    ...
    def __init__(
        ...
        *,
        ...
        self.pdb = self.debugger_cls() if call_pdb else None
    ...
    def _get_chained_exception(exception_value):
        if cause := getattr(exception_value, "__cause__", None):
            return cause
    ...
    def get_parts_of_chained_exception(
        ...
        if chained_evalue := self._get_chained_exception(evalue):
...
class FrameInfo:
    ...
    def variables_in_executing_piece(self):
        return self._sd.variables_in_executing_piece if self._sd else []
    ...
    @property
    def executing(self):
        return self._sd.executing if self._sd else None
...
class VerboseTB(TBTools):
    ...
    def format_record(self, frame_info: FrameInfo):
        ...
        if self.include_vars:
            try:
                ...
                lvals_list.extend(tpl_name_val % (var.name, repr(var.value)) for var in fibp)
            except Exception:
                lvals_list.append(
                    "Exception trying to inspect frame. No more locals available."
                )
        lvals = f'{indent}{em_normal.join(lvals_list)}' if lvals_list else ''
        ...
        if frame_info._sd is None:
            ...
            return f'{level}{"".join(_simple_format_traceback_lines(current_line, index, raw_lines, Colors, lvals, _line_format))}'
    ...
    def prepare_header(self, etype: str, long_version: bool = False):
        ...
        if long_version:
            ...
            # Simplified header
            head = f'{exc}{"Traceback (most recent call last)".rjust(width - len(etype))}'
    ...
    def format_exception(self, etype, evalue):
        ...
        return [
            ...
            *(
                f'{colorsnormal}{_safe_string(py3compat.cast_unicode(n), "note")}'
                for n in notes
            ),
        ]
...
class AutoFormattedTB(FormattedTB):
    ...
    def structured_traceback(
        ...
        self.tb = etb[0] if isinstance(etb, tuple) else etb
