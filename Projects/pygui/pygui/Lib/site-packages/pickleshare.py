...
class PickleShareLink:
    ...
    def __setattr__(self,key,val):
        self.db[f'{self.keydir}/{key}'] = val
    def __repr__(self):
        db = self.__dict__['db']
        keys = db.keys( self.__dict__['keydir'] +"/*")
        return f"""<PickleShareLink '{self.__dict__['keydir']}': {";".join([Path(k).basename() for k in keys])}>"""
